...
Every vote is HMAC-SHA256 authenticated with the enrolled validator key and binds the consensus domain/version, epoch, proposal ID, 256-bit proposal hash, validator identity, yes/no decision, and proposal timestamp. Changing a bound field invalidates that signature.

Session MAC authentication remains a transport/session control; the validator vote signature is a separate consensus-level binding.
...
With three validators, one yes vote stays pending, two yes votes accept, and two no votes reject because acceptance quorum is no longer reachable.
...
Once a proposal becomes `ACCEPTED` or `REJECTED`, its result is frozen and late votes are rejected. A proposal ID is bound to one proposal hash for the epoch, while each accepted vote independently binds and freshness-checks its proposal timestamp.
...
The Go process is a local authenticated control backend, not a consensus participant. v2.6 removes network identity-admission policy from its inherited runtime environment. Normal dashboard-facing identity/consensus metadata is sourced from the Python sync enforcement layer.
...
Accordingly, v2.6 continues to report `majority_attack_resistant = false` and `protects_against_forward_majority_control = false`.
...
The hosted security baseline covers outsider validator rejection, forged voter identity rejection, invalid vote signatures, duplicate votes, vote flips, full-set quorum, proposal-hash substitution, stale proposals, wrong epochs, epoch rotation, finalized-proposal freeze, global-PSK-only admission rejection, metadata redaction, all prior Python security suites, Go runtime tests, and the Go 1.22 build.

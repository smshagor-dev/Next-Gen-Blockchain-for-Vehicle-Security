...
The Vehicle Status and Network Status canvases use exact user-supplied reference
artwork encoded as local PNG data parts under assets/dashboard/reference/. No
runtime image generation or remote hotlink is used. All live values, controls,
metrics, blockchain state, and socket behavior remain inherited.
"""
...
def _load_reference_png(stem: str):
    """Decode a bundled PNG split into deterministic base64 text parts."""
...
def _fit_reference(image, width: int, height: int):
    """Contain the exact reference crop without distorting its aspect ratio."""
...
class RasterReferenceDashboard(ExactReferencePanelsDashboard):
    """Use exact reference PNG crops for vehicle and network artwork only."""
    ...
    def _draw_vehicle_art(self, canvas: tk.Canvas) -> None:
        """Render the exact supplied Vehicle Status reference crop."""
        ...
        try:
            ...
            image = _fit_reference(_load_reference_png("vehicle-status"), width, height)
    ...
    def _draw_peer_map(self, canvas: tk.Canvas, peers: Sequence[Any]) -> None:
        """Render the exact supplied Network Status reference map background.

        The map artwork is presentation-only. The four metrics below the map are
        still source-backed by the inherited live renderer and are never copied
        ...
        try:
            ...
            image = _fit_reference(_load_reference_png("network-status"), width, height)
